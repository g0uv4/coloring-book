...
這是一份可搬運的 `SKILL.md` 包，名稱 **`coloring-book`**。上傳照片後，代理會先出「保留／省略」清單，再把照片譯成 **Open-Line Plate** 線稿，列出已辨識的圖片元素，然後**停下等你確認**。只有你確認後才會合成 A4 PDF。

這是 **skill**。不要部署成應用程式。不要把多張照片合併成一本書。
